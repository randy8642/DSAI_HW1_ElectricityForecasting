...
if args.plot == "Fig_01":
    # Read data
    MW, perc, PP_MW, P_MW, _ = _Plot("training_data.csv")
    ...
    _, _, _, _, VAL_MW = _Plot("training_data.csv")
    dates = pd.date_range('2021-02-20', '2021-02-27').strftime("%Y-%m-%d").to_list()
    dates = [datetime.strptime(d, '%Y-%m-%d').date() for d in dates]
    data_f = args.data + "_" + "val_pred.npy"
    val_pred = np.load(data_f)
